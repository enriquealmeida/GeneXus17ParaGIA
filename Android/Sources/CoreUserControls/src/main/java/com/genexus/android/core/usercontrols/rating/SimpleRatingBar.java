 ...
 * Copied from https://github.com/FlyingPumba/SimpleRatingBar
 */
public class SimpleRatingBar extends View {
    ...
    public enum Gravity {
        ...
        static Gravity fromId(int id) {
            ...
            // default value
			Services.Log.warning("SimpleRatingBar", String.format("Gravity chosen is neither 'left' nor 'right', I will set it to Left"));
            return Left;
        }
    }
    ...
    private void parseAttrs(AttributeSet attrs) {
        ...
        borderColor = arr.getColor(R.styleable.SimpleRatingBar_srb_borderColor, ContextCompat.getColor(getContext(), R.color.srb_golden_stars));
    ...
    private void validateAttrs() {
        ...
        if (desiredStarSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && desiredStarSize
                > maxStarSize) {
			Services.Log.warning("SimpleRatingBar", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", desiredStarSize, maxStarSize));
        }
        if (stepSize <= 0) {
    ...
    public void setStarSize(float starSize) {
        ...
        if (starSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && starSize > maxStarSize) {
			Services.Log.warning("SimpleRatingBar", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", starSize, maxStarSize));
        }
        // force re-calculating the layout dimension
        requestLayout();
        generateInternalCanvas(getWidth(), getHeight());
    ...
    private float normalizeRating(float rating) {
        if (rating < 0) {
			Services.Log.warning("SimpleRatingBar", String.format("Assigned rating is less than 0 (%f < 0), I will set it to exactly 0", rating));
            return 0;
        } else if (rating > numberOfStars) {
			Services.Log.warning("SimpleRatingBar", String.format("Assigned rating is greater than numberOfStars (%f > %d), I will set it to exactly numberOfStars", rating, numberOfStars));
            return numberOfStars;
        } else {
            return rating;
        }
