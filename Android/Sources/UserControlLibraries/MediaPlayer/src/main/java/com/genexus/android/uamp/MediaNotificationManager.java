...
import com.genexus.android.uamp.utils.ResourceHelper;
...
public class MediaNotificationManager extends BroadcastReceiver {
    ...
    public static final String ACTION_CANCEL = "com.example.android.uamp.cancel";
    ...
    public MediaNotificationManager(MusicService service) throws RemoteException {
        ...
        mStopCastIntent = PendingIntent.getBroadcast(mService, REQUEST_CODE,
                new Intent(ACTION_STOP_CASTING).setPackage(pkg), flags);
        mCancelIntent = PendingIntent.getBroadcast(mService, REQUEST_CODE,
                new Intent(ACTION_CANCEL).setPackage(pkg), flags);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        switch (action) {
            ...
            case ACTION_STOP_CASTING:
                ...
                break;
            case ACTION_CANCEL:
                mTransportControls.stop(); // Also stops foreground service, and this notification
                break;
            default:
    ...
    private PendingIntent createContentIntent(MediaDescriptionCompat description) {
        Intent openUI = ApplicationAudioResource.getInstance().getContentIntent(description);
    ...
    private Notification createNotification() {
        LogHelper.d(TAG, "updateNotificationMetadata. mMetadata=" + mMetadata.getDescription());
        if (mMetadata == null || mPlaybackState == null) {
            return null;
        }

        NotificationCompat.Builder notificationBuilder = newBuilderPlayBack(mService);
	...
	public NotificationCompat.Builder newBuilderPlayBack(Context context) {
	...
	@RequiresApi(26)
	private void createMediaPlayerChannel(Context context) {
		NotificationManager notificationManager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
		if (notificationManager == null) {
			throw new IllegalStateException("Notification Service not available");
		}

		boolean channelExists = notificationManager.getNotificationChannel(CHANNEL_PLAYBACK_ID) != null;

		if (channelExists)
			return;

		// The id of the channel.
		String id = CHANNEL_PLAYBACK_ID;
