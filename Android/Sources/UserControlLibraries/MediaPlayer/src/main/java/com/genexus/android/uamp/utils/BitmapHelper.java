...
public class BitmapHelper {
    ...
    public static Bitmap fetchAndRescaleBitmap(String uri, int width, int height)
            throws IOException
	{
		LogHelper.d(TAG, "fetchAndRescaleBitmap :  ", uri);
        URL url = new URL(uri);
        ...
        try {
			...
			catch (IOException ex)  // could get "Mark has been invalidated" in 4.x device and some images, retry
			{
				// from: https://learnandroidtoday.wordpress.com/2013/08/27/android-java-io-ioexception-mark-has-been-invalidated/
				if (is != null)
				{
					is.close();
