...
public class Utils {
    ...
    public static void showErrorDialog(Context context, String errorString) {
        new AlertDialog.Builder(context).setTitle(R.string.GXM_errtitle)
    ...
    public static void showQueuePopup(final Context context, View view, final MediaInfo mediaInfo) {
        ...
        if (castSession == null || !castSession.isConnected()) {
			Services.Log.warning(TAG, "showQueuePopup(): not connected to a cast device");
            return;
        }
        final RemoteMediaClient remoteMediaClient = castSession.getRemoteMediaClient();
        if (remoteMediaClient == null) {
			Services.Log.warning(TAG, "showQueuePopup(): null RemoteMediaClient");
            return;
        }
