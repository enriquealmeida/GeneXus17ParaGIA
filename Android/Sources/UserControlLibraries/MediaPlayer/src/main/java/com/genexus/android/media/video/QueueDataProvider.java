...
public class QueueDataProvider {
    ...
    public MediaQueueItem getUpcomingItem() {
		Services.Log.debug(TAG, "[upcoming] getUpcomingItem() returning " + mUpcomingItem);
    ...
    private class MyRemoteMediaClientListener implements RemoteMediaClient.Listener {
        ...
        public void onPreloadStatusUpdated() {
            ...
            mUpcomingItem = mediaStatus.getQueueItemById(mediaStatus.getPreloadedItemId());
			Services.Log.debug(TAG, "onRemoteMediaPreloadStatusUpdated() with item=" + mUpcomingItem);
            if (mListener != null) {
                mListener.onQueueDataChanged();
            }
        }
        ...
        public void onQueueStatusUpdated() {
			...
			Services.Log.debug(TAG, "Queue was updated");
        }
        ...
        private void updateMediaQueue() {
            ...
            mQueue.clear();
            if (queueItems == null) {
				Services.Log.debug(TAG, "Queue is cleared");
            } else {
				Services.Log.debug(TAG, "Queue is updated with a list of size: " + queueItems.size());
    ...
    private RemoteMediaClient getRemoteMediaClient() {
        ...
        if (castSession == null || !castSession.isConnected()) {
			Services.Log.warning(TAG, "Trying to get a RemoteMediaClient when no CastSession is started.");
            return null;
        }
