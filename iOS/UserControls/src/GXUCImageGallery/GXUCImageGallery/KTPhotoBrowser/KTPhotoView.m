...
- (id)initWithFrame:(CGRect)frame
{
	...
	if (self) {
		...
		[self setMaximumZoomScale:5.0];[self setShowsHorizontalScrollIndicator:NO];
...
{
	[imageView_ gxCancelCurrentImageLoad];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if ([touch view] == self) {
		if ([touch tapCount] == 2) {
			...
			if (_touchDelegate) {
				[NSObject cancelPreviousPerformRequestsWithTarget:_touchDelegate selector:@selector(photoViewDidReceiveTouch:) object:self];
			}
...
#define kDoubleTapWaitInterval 0.4

- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	UITouch *touch = [touches anyObject];
	if ([touch view] == self) {
		if ([touch tapCount] == 1) {
			[self performSelector:@selector(toggleChromeDisplay) withObject:nil afterDelay:kDoubleTapWaitInterval];
			if (_touchDelegate) {
				[_touchDelegate performSelector:@selector(photoViewDidReceiveTouch:) withObject:self afterDelay:kDoubleTapWaitInterval];
			}
		}
	}
}
...
@implementation KTPhotoView (GXWebImage)

- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder fullSize:(BOOL)full {
	GXWebImageOptions options = full ? GXWebImageFullSizeImage : 0;
	[imageView_ gxSetImageWithURL:url placeholderImage:placeholder options:options];
}
