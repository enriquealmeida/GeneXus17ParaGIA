...
const i = class {
  ...
  itemClickedHandler(t) {
    t.stopPropagation(), this.accordions.forEach(e => {
      const o = e.itemId;
      this.singleItemOpen ? o === t.detail ? "open" === e.status ? e.status = "closed" : e.status = "open" : e.status = "closed" : o === t.detail && ("open" === e.status ? e.status = "closed" : e.status = "open");
    });
  }
  itemLoadedHandler() {
  ...
  setupAccordions() {
    if (this.accordions = Array.from(this.el.querySelectorAll(":scope > gxg-accordion-item")), 
    //Disabled
    this.disabled && this.accordions.forEach(t => {
      t.status = "closed";
    }), this.singleItemOpen) {
      /* If "single-item-open" is true, and more than one accordion has the "open" property,
            show only the first accordion open.*/
      let t = 0;
      if (this.accordions.forEach(e => {
        "open" === e.getAttribute("status") && t++;
      }), t > 1) {
        let t = !1;
        this.accordions.forEach(e => {
          "open" === e.getAttribute("status") && (t ? e.setAttribute("status", "closed") : t = !0);
        });
      }
    }
    this.accordions.forEach(t => {
      t.setAttribute("mode", this.mode), this.noPadding && t.setAttribute("no-padding", ""), 
      this.disabled && t.setAttribute("disabled", "disabled");
    });
  }
  render() {
    return e(o, {
