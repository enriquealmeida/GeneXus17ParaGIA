...
/**@preserve GeneXus 15.0.4.113719*/
	...
	return {
		...
		csv: {
			...
			ctxControlId: function (ControlId, gxO) {
				...
				if (cmpContext) {
					if (ControlId.indexOf(cmpContext) == 0) {
						return ControlId;
					}
				}
				...
				loadScript: function (url, callback, ignoreExisting) {
					...
					if (typeof (scriptCallback) == 'function') {
						if (script.addEventListener)
							gx.evt.attach(script, ['load', 'error'] ,scriptCallback, { userCapture: false} );
						else {
...
var GlobalEvents = (function ($) {
	return {		
		executeMethod: function(methodName, parms) {			
			...
			try {
				if (window.top && window.top !== window && window.top.gx) {
					window.top.gx.fx.obs.notify(methodName, parms);
				}
			}
			catch (e) {}
		}
...
gx.plugdesign = (function($) {
	...
	return {
		...
		getElements: function (t, selector, excluded) {
			var $elements,
				selectorIsJQuery = (selector instanceof $),
				localSelector;

			if (typeof t.selector == "function")
			{
				$elements = t.selector(selectorIsJQuery || !selector ? "" : selector);
			}
			else
			{
				localSelector = selector && !selectorIsJQuery && !t.global ? (selector + " " + t.selector.split(',').join(", " + selector)) : t.selector;
		...
		controls: (function () {
			...
			var templates = {},
				...
				applyTemplate = function (name, context) {
					var source;
					if (!templates[name]) {
						source = gx.lang.isArray(templatesSource[name]) ? templatesSource[name].join("") : templatesSource[name];
				...
				radio: function (id, width, height, cssClass, title) {
					...
					this._getHtml = function () {
						...
						this.append(applyTemplate("radio", {
							...
							gxoch0: gx.html.controls.onJSEvent_impl('gxoch0', sEventJsCode, this.usrOnclick),
	...
	return {
		...
		Dialog: function () {
			...
			this.open = function (cfg) {
				...
				gx.popup.ext.window(this, false, 0, 0, cfg.w, cfg.h, cfg.contentHtml, "white", cfg.title, "black", "black", "black", "black", true, true, true, resizable, false, gx.ajax.getImageUrl(gx, 'resizeImage'), POPUP_HEADER_CLASS, POPUP_CLOSE_CLASS, POPUP_CONTENT_CLASS, popupClass, "PopupShadow", cfg.showParentPopups, false, cfg.callbacks, cfg.parentElement);
		...
		ext: {													
			...
			close: function (popupObj, beforeClose) {
				...
				try {
					if (gx.util.browser.isSafari() && popupObj.initialScroll && popupObj.initialScroll.scrollTop != -1)
						window.scrollTo(popupObj.initialScroll.scrollLeft, popupObj.initialScroll.scrollTop);
				}
				catch (e) {}
			...
			window_responsive: function (popupObj, autoSize, x, y, w, h, contentHtml, bgcolor, title, titlecolor, bordercolor, scrollcolor, shadowcolor, ismodal, showonstart, isdrag, isresize, isExt, rsImg, headCls, btnCls, cntCls, brdCls, shdwCls, showParentPopups, swallowKeys, callbacks, parentElement) {
				...
				popupObj.initialScroll = gx.dom.documentScroll(this.doc);
			...
			if (Commands && Commands.length > 0) {
				...
				for (var i = 0; i < len; i++) {
					...
					if (Command.refresh !== undefined || Command.refresh_form !== undefined || Command.cmp_refresh === '') {
						gx.ajax.dispatchRefreshCommand(Command, gxO);
					}
					if (Command.redirect) {
				...
				for (var i = 0; i < varsLen; i++) {
					...
					if (typeof (vStruct.rfrVar) != 'undefined') {
						...
						if (!gx.lang.emptyObject(vStruct.hc)) {
							...
							refreshParms += encodeURIComponent(v) + ",";							
						}
						else
		...
		this.appendHeader = function (visibleColumnsArray, buffer) {
			...
			for (var i = 0; i < colsLen; i++) {
				var column = visibleColumnsArray[i],
					columnClass = [this.columnHeaderClass, column.gxColumnClass, column.columnclass].join(" ");
...
gx.GxObject = (function($) {
	var gxObject = function () {
		...
		this.cmpRegex = gx.GxObject.CONTROL_CMP_REGEX;
		...
		this.getCmpType = function (cmpContext) {
		...
		this.executeEnterEvent = function (evt, ctrl, disableForm) {
		...
		this.postbackLoad = function (userControls, newComponents) {
			if (this.hasMasterPage())
						...
						if (typeof (gxValues[Property]) != "function") {
							...
							if (validStruct) {
								if (validStruct.v2v && !gx.fn.autoRefreshingControl(validStruct.fld, cmpCtx)) {
									...
									if (validStruct.grid != 0 ) {
										row = gx.fn.controlRowId(gx.dom.el(Property)) || gx.fn.currentGridRowImpl(validStruct.grid);
									}
									if (!additiveResponse) {
		...
		setErrorViewer: function (Messages) {
			...
			try {
				...
				for (var cmpCtx in Messages) {
					...
					gx.plugdesign.applyTemplateObject({
						templateSelector: function(t) { return t.name === 'errorviewer'; }
					});
				}
			}
			catch (e) {
			...
			if (!found) {
				$.each(cssRules, function( i, r) {
					if (r && r.selectorText && normalizeCompare(r.selectorText, selector.replace(':', '::') + ',', function(a,b){ return a.indexOf(b) == 0})) {
						styleSheet.deleteRule( i);
						idx = i;
						styleSheet.insertRule( r.selectorText + cssCode, idx);
						partial = true;
					}
				});
			}
			if (!partial) {
				styleSheet.insertRule( selector + cssCode, idx);
			}
		}
