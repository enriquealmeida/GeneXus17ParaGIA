...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'Scanner'">
        <xsl:call-template name="RenderScanner"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- Scanner design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderScanner">
    <span atomicselection="true">
      <xsl:call-template name="AddStyleAttribute"/>
	  	This control is not supported in this platform. 
    </span>
  </xsl:template>
