...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'Rating'">
        <xsl:call-template name="RenderRating"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- Rating design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderRating">
    <span atomicselection="true">
      <xsl:call-template name="AddStyleAttribute"/>
	   <img style='width:82px;height:18px;'>
        <xsl:attribute name="src">
          <xsl:value-of select='gxca:GetMyPath()'/>
							<xsl:text>\RatingRender.png</xsl:text>
        </xsl:attribute>
      </img>
    </span>
  </xsl:template>
