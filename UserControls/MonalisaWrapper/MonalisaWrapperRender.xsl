...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'MonalisaWrapper'">
        <xsl:call-template name="RenderMonalisaWrapper"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- MonalisaWrapper design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderMonalisaWrapper">
    <span atomicselection="true">
      <xsl:call-template name="AddStyleAttribute"/>
       <img>
        <xsl:attribute name="src">
          <xsl:value-of select="gxca:GetMyPath()"/>
          <xsl:text>\Monalisa.png</xsl:text>
        </xsl:attribute>
      </img>
