...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'GoogleGadget'">
        <xsl:call-template name="RenderGoogleGadget"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- GoogleGadget design render -->
  <xsl:template name="RenderGoogleGadget">
    ...
    <span atomicselection="true">
      ...
      <img>
        <xsl:attribute name="src">
          ...
          <xsl:text>\GoogleGadget\google.gif</xsl:text>
        </xsl:attribute>
        <xsl:call-template name="AddStyleAttribute"/>
      </img>
    </span>
