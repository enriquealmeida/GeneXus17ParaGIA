...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'GoogleCharts'">
        <xsl:call-template name="RenderGoogleCharts"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- GeneXusColumnChart design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderGoogleCharts">
			...
			<TR>
				<TD>
					<xsl:attribute name="background">
						...
						<xsl:text>\GoogleCharts\GoogleCharts.png</xsl:text>
					</xsl:attribute>
				</TD>
			</TR>
		</table>		          
