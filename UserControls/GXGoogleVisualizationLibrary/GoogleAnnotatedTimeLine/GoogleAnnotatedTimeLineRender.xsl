...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'GoogleAnnotatedTimeLine'">
        <xsl:call-template name="RenderGoogleAnnotatedTimeLine"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- GoogleAnnotatedTimeLine design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderGoogleAnnotatedTimeLine">
		...
		<table width="100%" height="100%" border="0">
			...
			<TD>            
				<xsl:attribute name="background">
					...
					<xsl:text>\GoogleAnnotatedTimeLine\AnnotatedTimeLine.PNG</xsl:text>
				</xsl:attribute>
			</TD>
			</TR>
     	</table>       
