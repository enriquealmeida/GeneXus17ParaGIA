...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'GoogleGauge'">
        <xsl:call-template name="RenderGoogleGauge"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- GoogleGauge design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderGoogleGauge">
				...
				<TR>
					<TD>
						<xsl:attribute name="background">
							...
							<xsl:text>\GoogleGauge\GoogleGauge.PNG</xsl:text>
						</xsl:attribute>
					</TD>
				</TR>
			</table>
