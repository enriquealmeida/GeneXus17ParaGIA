...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'GoogleMotion'">
        <xsl:call-template name="RenderGoogleMotion"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- GoogleMotion design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderGoogleMotion">
					...
					<TR>
						<TD>
							<xsl:attribute name="background">
								...
								<xsl:text>\GoogleMotion\GoogleMotionChart.PNG</xsl:text>
							</xsl:attribute>
						</TD>
					</TR>
				</table>
