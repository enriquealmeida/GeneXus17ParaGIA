...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'OrganizationalChart'">
        <xsl:call-template name="RenderOrganizationalChart"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- OrganizationalChart design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderOrganizationalChart">
		...
		<table width="100%" height="100%" border="0">
			...
			<TD>            
				<xsl:attribute name="background">
					...
					<xsl:text>\OrganizationalChart\OrganizationalChart.PNG</xsl:text>
				</xsl:attribute>
			</TD>
			</TR>
     	</table>       
