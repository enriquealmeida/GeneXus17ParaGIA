...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'GoogleGeoMap'">
        <xsl:call-template name="RenderGoogleGeoMap"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- GeneXusGeoMap design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderGoogleGeoMap">
			...
			<TR>
				<TD>
					<xsl:attribute name="background">
						...
						<xsl:text>\GoogleGeoMap\GoogleGeoMap.bmp</xsl:text>
					</xsl:attribute>
				</TD>
			</TR>
		</table>		          
