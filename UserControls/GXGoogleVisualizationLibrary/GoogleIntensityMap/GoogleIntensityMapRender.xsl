...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'GoogleIntensityMap'">
        <xsl:call-template name="RenderGoogleIntensityMap"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- GoogleIntensityMap design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderGoogleIntensityMap">
					...
					<TR>
						<TD>
							<xsl:attribute name="background">
								...
								<xsl:text>\GoogleIntensityMap\GoogleIntensityMap.PNG</xsl:text>
							</xsl:attribute>
						</TD>
					</TR>
				</table>
