...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'gpxreCAPTCHA'">
        <xsl:call-template name="RendergpxreCAPTCHA"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- gpxreCAPTCHA design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RendergpxreCAPTCHA">
    <span atomicselection="true">
	  ...
	  <img>
        <xsl:attribute name="src">
          ...
          <xsl:text>\gpxrecaptcha.png</xsl:text>
        </xsl:attribute>
        <xsl:call-template name="AddStyleAttribute"/>
      </img>
