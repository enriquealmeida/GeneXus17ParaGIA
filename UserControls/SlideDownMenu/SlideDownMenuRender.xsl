...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'SlideDownMenu'">
        <xsl:call-template name="RenderSlideDownMenu"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- SlideDownMenu design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderSlideDownMenu">
        ...
        <TR>
          <TD>            
              <xsl:attribute name="background">
                ...
                <xsl:text>\slidedownmenu.png</xsl:text>
              </xsl:attribute>
          </TD>
        </TR>
      </table>      
