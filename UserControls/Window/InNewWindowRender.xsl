...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'InNewWindow'">
        <xsl:call-template name="RenderInNewWindow"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- InNewWindow design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderInNewWindow">
				...
				<TR>
					<TD>            
						<xsl:attribute name="background">
										...
										<xsl:text>\windowopen.png</xsl:text>
						</xsl:attribute>
