...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'ActionGroupDynamicItems'">
        <xsl:call-template name="RenderDynamicItems"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- DynamicItems design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderDynamicItems">
    <span atomicselection="true">
      <xsl:call-template name="AddStyleAttribute"/>
		  Action Group Dynamic Items
    </span>
  </xsl:template>
