...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'reCAPTCHA'">
        <xsl:call-template name="RenderreCAPTCHA"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- reCAPTCHA design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderreCAPTCHA">
    <span atomicselection="true">
	  ...
	  <img>
        <xsl:attribute name="src">
          ...
          <xsl:text>\recaptcha.png</xsl:text>
        </xsl:attribute>
        <xsl:call-template name="AddStyleAttribute"/>
      </img>
