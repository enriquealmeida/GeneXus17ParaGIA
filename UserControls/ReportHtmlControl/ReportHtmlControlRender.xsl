...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'ReportHtmlControl'">
        <xsl:call-template name="RenderReportHtmlControl"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- ReportHtmlControl design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderReportHtmlControl">
    <span atomicselection="true">
      <xsl:call-template name="AddStyleAttribute"/>
      
     	<table width="123" height="123" border="0">
			<TR>
				<TD>            
					<xsl:attribute name="background">
						<xsl:value-of select='gxca:GetMyPath()'/>
						<xsl:text>\ReportHtmlControl_Logo.png</xsl:text>
					</xsl:attribute>
