  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'DVelop.Bootstrap.TagsInput'">
        <xsl:call-template name="RenderDVelop.Bootstrap.TagsInput"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- DVelop.Bootstrap.TagsInput design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderDVelop.Bootstrap.TagsInput">
    <span atomicselection="true">
      <xsl:call-template name="AddStyleAttribute"/>
      <img alt="TagsInput control">
