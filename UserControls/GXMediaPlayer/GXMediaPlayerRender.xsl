...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'GXMediaPlayer'">
        <xsl:call-template name="RenderGXMediaPlayer"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- GXMediaPlayer design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderGXMediaPlayer">
		...
		<TR>
			<TD>
				<xsl:attribute name="background">
					...
					<xsl:text>\GXMediaPlayer.png</xsl:text>
				</xsl:attribute>
			</TD>
		</TR>
	  </table>	
