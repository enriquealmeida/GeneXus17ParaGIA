...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'scaHalfStarRating'">
        <xsl:call-template name="RenderscaHalfStarRating"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- StarRating design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderscaHalfStarRating">
    <span atomicselection="true">
      <xsl:call-template name="AddStyleAttribute"/>
	  	<img>
			<xsl:attribute name ="src">
            	<xsl:value-of select='gxca:GetMyPath()'/>
				<xsl:text>\design\scaHalfStarRatingRender.png</xsl:text>
              </xsl:attribute>
		</img>
    </span>
  </xsl:template>
